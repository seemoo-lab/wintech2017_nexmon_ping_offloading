...
wl_arp_recv_proc_hook(struct wlc_info **arpi, struct sk_buff *p)
{
    ...
    if (frame->ip.protocol == IP_PROTOCOL_ICMP && frame->icmp.type == ICMP_TYPE_REQUEST) {
        //printf("%s: %d %08x\n", __FUNCTION__, i++, p->rspec);
        
        // Override the rate setting for the ping reply with the rate of the ping request frame
        if (ping_offloading & 2) {
            wlc->band->rspec_override = p->rspec | RATES_OVERRIDE_RATE | RATES_OVERRIDE_MODE;
        }

        if (ping_offloading & 1) {
